...
% 6.1 计算映射缩放比例
scale_ratio = dp_focal / (cam_pixel_pitch / 1000); % 物理像素缩放比 (~6.69)
algo_roi_h = row_end - row_start + 1;
algo_roi_w = col_end - col_start + 1;
cam_roi_h = round(algo_roi_h * scale_ratio);
cam_roi_w = round(algo_roi_w * scale_ratio);

% 理论预测中心 (仅作备用与显示)
theory_cx = round(cx + shift_x * scale_ratio);
theory_cy = round(cy - shift_y * scale_ratio); 

% ---------------------------------------------------------
% 【全新功能】：基于图像形态学直接寻找平顶光外接矩形 (Bounding Box)
% ---------------------------------------------------------
fprintf('--> 正在通过图像形态学直接锁定平顶光矩形区域...\n');
% 1. 全图屏蔽零级光，提取出纯净的寻星图
search_img = double(initial_flattop) .* mask_zero;

% 2. 自适应二值化 (截取最高亮度的 30% 轮廓)
bg_threshold = max(search_img(:)) * 0.3; 
bw_img = search_img > bg_threshold;

% 3. 形态学滤波：干掉离散的噪点 (只保留面积大于 1000 像素的亮斑)
bw_img = bwareaopen(bw_img, 1000);

% 4. 寻找所有连通域的包围盒
props = regionprops(bw_img, 'BoundingBox', 'Area');
if isempty(props)
    warning('未能在画面中提取到平顶光轮廓，退回理论坐标！');
    cam_cx = theory_cx; cam_cy = theory_cy;
else
    % 找到面积最大的那块亮斑（即我们的目标平顶光）
    [~, max_idx] = max([props.Area]);
    bbox = props(max_idx).BoundingBox; % 返回格式: [x, y, width, height]
    
    % 取这个物理外接矩形的绝对几何中心
    cam_cx = round(bbox(1) + bbox(3)/2);
    cam_cy = round(bbox(2) + bbox(4)/2);
    
    fprintf('    理论预测中心: (X: %d, Y: %d)\n', theory_cx, theory_cy);
    fprintf('    形态学锁定中心: (X: %d, Y: %d) [偏移量: dx=%d, dy=%d]\n', ...
            cam_cx, cam_cy, cam_cx - theory_cx, cam_cy - theory_cy);
end
% ---------------------------------------------------------

% 6.2 根据“真实锁定”的中心，圈定严丝合缝的 ROI 准备闭环
cam_r_start = max(1, cam_cy - round(cam_roi_h/2));
cam_r_end   = min(cam_H, cam_r_start + cam_roi_h - 1);
cam_c_start = max(1, cam_cx - round(cam_roi_w/2));
cam_c_end   = min(cam_W, cam_c_start + cam_roi_w - 1);
